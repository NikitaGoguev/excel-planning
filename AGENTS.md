...
- `100_Шаблон экспресс оценки`: visible sheet copied from `assets/Шаблон Экспресс оценки.xlsx`, placed after `99_Правила планирования`, and used as the formatted template for per-root express-estimate exports.
- `101_Списки`: visible editable source tables `tblTaskCommentArtifacts` and `tblTaskCommentAdjacentTeams`, plus `tblStatuses` in `E3:E10` and `tblYesNo` in `G3:G5`.
- `99_Правила планирования`: expertise list `AN`, `DE`, `BE`, `FE`, `QA`, waterfall/order rules, planning rule descriptions, and editable planning rule settings table `tblPlanningRuleSettings`. Editable status and yes/no lists live on `101_Списки`.

## Important Design Decisions

- Keep the workbook as `.xlsx` without macros unless macro-enabled automation is explicitly needed.
- Use only expertise codes `DE`, `AN`, `BE`, `FE`, `QA`.
- Base delivery waterfall remains `AN -> BE -> FE -> QA`; DE starts in parallel with AN and does not add downstream dependencies.
...
- `tblTeamComposition` spans `A13:B20`; role comments are merged UI fields in `C13:G20` and are intentionally outside the ListObject. `Дизайнер` follows `Тестировщик`.
...
- `E23:E24` are the effective designer count and vacation person-days, `F23:F24` are employee-derived auto values, and `G23:G24` are manual overrides.
- `E25` is the separate designer focus factor (default `70%`), `E34` is designer capacity, and `E35` is the total capacity including designers; legacy core-team total `E32` remains unchanged.
...
- Keep core formulas on `E27:E30`; designer capacity is in `E34`, and sheet 04 references the effective `E` values.
...
- `J2:O4` contains the formula-driven resource balance block: labels in `J2:J4` (`Емкость`, `План+риск`, `Баланс`) and DE/AN/BE/FE/QA values in `K:O`; the risk value stays configurable in `99_Правила планирования!tblPlanningRuleSettings` and is looked up by rule code rather than a fixed cell.
...
- With the default config, `tblPlanBacklog` is `A55:T155` and the bulk action is `A54`. Both backlog and sheet 03 capacities derive from the same `taskRows` value.
- `RunQuarterPlanRecalculate` recalculates dates only for active plan rows through the Mac-safe scheduling path.
- `G2` (`Загрузить 03 в бэклог`), `Q2` (`Посчитать план`), and `R2` (`Экспорт плана`) remain shape-backed buttons bound to stable public macros.
- `JQL` action buttons are shape-backed at the layout-derived cells for active plan, grey zone, and backlog (defaults `I4`, `I29`, and `I54`). They call the plain public macro `RunCopyJql` and copy `issuekey in (...)` from the corresponding table's `ЗНИ/Jira` column.
- The grey-zone and backlog JQL action rows must be derived from `config/workbook-limits.json`; VBA uses generated `QP_*_JQL_ACTION_CELL` constants rather than handwritten row numbers.
- `RunQuarterPlanExportBacklog` exports non-empty `tblPlanActive` rows to a standalone `.xlsx`, writing only visible user columns `F:T` and the balance block `J:O`. The default filename is `<Команда>_<Год>_Q<Квартал>_квартальный план_<yyyy-mm-dd_hh-nn>.xlsx`, with team/year/quarter read from `00_Настройки`.
...
- Technical schedule columns `U:AJ` are visible by design and must not be hidden; they show DE/AN/BE/FE/QA durations, dates, and diagnostics.
...
- `Статус на конец квартала` uses `101_Списки!tblStatuses`; allowed values are `Готова аналитика`, `Готова разработка`, `Готова разработка (бэк)`, `Готова разработка (фронт)`, `Готово к релизу`, `ПРОМ`, and `Отложено`. Blank status is treated as `ПРОМ` for resource balance, schedule calculation, and excluded-estimate marking.
- Status controls which core stages consume resources and appear in technical schedule columns: `Готова аналитика` calculates only AN, `Готова разработка` calculates AN/BE/FE, `Готова разработка (бэк)` calculates AN/BE, `Готова разработка (фронт)` calculates AN/FE, blank/`Готово к релизу`/`ПРОМ` calculates the full AN/BE/FE/QA path, and `Отложено` calculates no stages. DE is included for every status except `Отложено`. `Трудозатраты` always remains the full normalized DE+AN+BE+FE+QA effort.
...
- `O3` is `Экспорт`; `O4:O103` contains shape-backed per-root export actions. `Примечание` is a visible user field in column `G`, DE is in `H`, and AN/BE/FE/QA occupy `I:L`.
...
- `H1:L1` contains the merged title `Статистика`; `H2:L2` contains formula-driven totals for `DE`, `AN`, `BE`, `FE`, and `QA` from `tblTaskEstimates`.
...
- `C1` is a shape-backed `Экспорт` button. It exports the user fields from `C:M` to a standalone `.xlsx` without action columns, with parent estimates written as values and a default filename based on `02_Capacity!E3` plus export date/time. XLSX import also accepts the former AN/BE/FE/QA format and leaves DE blank for it.
- `F2` is a shape-backed `JQL` button bound to `RunCopyJql`. It copies only root-task `ЗНИ/Jira` values in table order; blank values and case-insensitive duplicates are excluded.
- JQL clipboard staging uses the layout-derived cell in visible column `Z` on `99_Правила планирования` and `Range.Copy`. Do not hide the source column: Excel copies a hidden cell only to its internal clipboard and exposes an empty plain-text value to browsers and other applications. Keep this path free of ActiveX, `DataObject`, Windows API, and `CreateObject`, preserve `ThisWorkbook.Saved`, and leave the clipboard unchanged when no keys exist or the text exceeds Excel's 32,767-character cell limit.
- Per-root `O` export actions create an `.xlsx` from the visible `100_Шаблон экспресс оценки` sheet. The default filename is `<ЗНИ/Jira> Экспресс-оценка.xlsx`; values are written to `E2:K2` and `E3:K3`, and root/child/grandchild rows are written from row 17 with descriptions in `A:D`, `B:D`, or `C:D`; AN maps to `G`, DE maps to the existing template field `Design` in `H`, and BE/FE/QA map to `I:K`.
...
- Compact action captions are `>` to create a child task, `x` to remove a decomposition subtree, and `Экспорт` in column `O` to export the root express estimate. VBA recreates shape buttons over these action cells on sheet activation or through `RunTaskEstimateRepairActionButtons`, using the same public-macro `OnAction` pattern as sheet 04. Do not switch these back to Unicode symbols inside/near the ListObject without testing Excel activation on Windows and Mac.
...
- Estimates are entered only on leaf rows. Parent rows on sheet 03 show formula-backed DE/AN/BE/FE/QA sums from direct children with a grey calculated style; child rows with grandchildren behave the same way.
- If `>` is used on a row with DE/AN/BE/FE/QA estimates, those estimates move to the new child row and are cleared from the parent.
...
- During load to sheet 04, only root tasks are loaded. For decomposed roots, DE/AN/BE/FE/QA values are calculated from leaf descendants; parent formulas on sheet 03 display direct-child sums for users.

## Planning Rules

- Task estimates are in person-days.
- `Трудозатраты` uses `CEILING(DE / designer focus factor, 1)` plus `CEILING(estimate / core team focus factor, 1)` for each AN/BE/FE/QA stage and is not inflated by allocation or vacation pauses.
- Technical stage durations and ready dates are calculated by the sheet 04 employee scheduler from `tblTeamMembers`, so low allocation and vacation gaps can increase `U:Y` and calendar dates without changing normalized effort.
- Empty DE/AN/BE/FE/QA estimates count as zero.
- Numeric overlap settings are read from `99_Правила планирования!tblPlanningRuleSettings` by code; do not hardcode these values or fixed cells in VBA.
...
- DE resources include designers. DE starts from the same earliest date as AN (or the task's first work date when AN is empty), reserves designers across tasks, and does not constrain BE/FE/QA or `Завершение (план)`.
- DE daily progress is designer allocation multiplied by `02_Capacity!E25`; the core stages continue to use `E20`.
- Resource balance uses only `tblPlanActive`: capacity from `02_Capacity!E34` and `E27:E30` minus active-plan DE/AN/BE/FE/QA estimates increased by `RESOURCE_BALANCE_RISK_PERCENT`.
- Greedy allocation picks the currently available employee of the required expertise with maximum allocation; equal allocations are resolved by the first row in `tblTeamMembers`.
...
- `npm run verify:scheduler` replaces the scheduler in a temporary XLSM copy with the current source, imports a test-only module, and compares all 18 output fields across exactly 63 pure-engine business scenarios; it is included in `verify:excel`. Desktop COM acceptance separately rejects an embedded scheduler that is stale relative to source.
