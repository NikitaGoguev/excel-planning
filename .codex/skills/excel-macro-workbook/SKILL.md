...
- Sheet 03 JQL includes only root rows. All JQL actions trim keys, skip blanks, remove case-insensitive duplicates in table order, and copy `issuekey in (...)` through the layout-derived staging cell in visible column `Z` on `99_Справочники`, without ActiveX or platform APIs. Keep the staging column visible because Excel exposes only an empty plain-text clipboard value for a copied hidden cell.
