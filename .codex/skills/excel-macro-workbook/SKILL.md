...
6. Validate the output:
   ...
   For the isolated 63-scenario exact-output domain scheduler gate, use:
...
- The scheduler COM harness replaces the module in a temporary XLSM with the current `assets/vba/QuarterPlanScheduler_module.txt` source before running all 63 scenarios, so `verify:scheduler` does not depend on a prior VBA sync. Desktop COM acceptance separately requires the embedded scheduler to match that source.
...
- Hidden header dropdowns are contract-enforced for selected ListObject columns: settings/team headers, sheet 00 employee headers, holiday headers, sheet 03 user estimate columns `G:M`, and sheet 04 action/status/comment/resource columns `A:E` and `J:T` across all plan sections.
- Sheet `00_Настройки` keeps `tblTeamComposition` at `A13:B20`, including `Дизайнер` after `Тестировщик`; `tblTeamMembers` starts at `A23:L23`, and its data-row capacity comes from `teamMembers` (default range `A23:L43`).
...
- Sheet `02_Capacity!E8:E19` are effective core-team counts and vacation person-days; designer count/vacation use `E23:E24`, auto values `F23:F24`, and overrides `G23:G24`. `E25` is the separate designer focus factor. Designer capacity is `E34`, while `E35` adds it to the legacy core-team total in `E32`.
...
- Planning overlap parameters are workbook settings in `99_Правила планирования!tblPlanningRuleSettings`; keep VBA reading these values by rule code instead of hardcoding numeric rule constants or fixed cells.
...
- `04_Квартальный план!J2:O4` contains the formula-driven resource balance block. Keep labels in `J2:J4` (`Емкость`, `План+риск`, `Баланс`), DE/AN/BE/FE/QA values in `K:O`, and formulas based on `tblPlanActive`, `02_Capacity!E34`/`E27:E30`, and `RESOURCE_BALANCE_RISK_PERCENT`. Do not show the risk as a separate top-row cell. The user-facing ready-date header is `Завершение (план)`.
- `04_Квартальный план` technical schedule columns `U:AJ` are user-visible and must not be hidden; they show DE/AN/BE/FE/QA durations, dates, and diagnostics.
...
- Sheet 04 scheduling uses one available employee per stage per working day. Core-stage progress is `Аллокация * 02_Capacity!E20`; DE progress uses `Аллокация * 02_Capacity!E25`. Full-day vacations block that employee, and another employee with the same expertise may continue the remaining work. Greedy selection chooses the available employee with maximum allocation; ties use the first row in `tblTeamMembers`.
- DE starts from the same earliest date as AN (or the task's first available date when AN is empty), reserves designers between tasks, and does not constrain BE/FE/QA or `Завершение (план)`. Invalid `E25` blocks scheduling only when a non-deferred task has a nonzero DE estimate.
- Default ranges remain `tblTaskEstimates=C3:M103`, `tblPlanBacklog=A55:T155`, and bulk action `A54`, but they are derived from `taskRows`; never duplicate these literals in builder, handwritten VBA, or verifiers.
- On workbook open, `04_Квартальный план` recreates shape buttons over `G2`, `Q2`, and `R2` and binds them to the stable public macros.
- `RunCopyJql` is the stable public entrypoint for the four JQL copy buttons. Their layout-derived anchor cells default to `03_Оценка задач!F2` and `04_Квартальный план!I4`, `I29`, and `I54`; grey-zone and backlog rows move with the configured section capacities.
- Sheet 03 JQL includes only root rows. All JQL actions trim keys, skip blanks, remove case-insensitive duplicates in table order, and copy `issuekey in (...)` through the layout-derived staging cell in visible column `Z` on `99_Правила планирования`, without ActiveX or platform APIs. Keep the staging column visible because Excel exposes only an empty plain-text clipboard value for a copied hidden cell.
...
- `03_Оценка задач!O3` is `Экспорт`; `O4:O103` contains shape-backed per-root express-estimate export actions. `Примечание` is a visible user field in column `G`, DE is in `H`, and AN/BE/FE/QA occupy `I:L`.
...
- `03_Оценка задач!H1:L1` is a merged `Статистика` title block; `H2:L2` must contain formula-driven totals for `DE`, `AN`, `BE`, `FE`, and `QA` based on `tblTaskEstimates`.
...
- Root-row `O` export actions must use the visible sheet `100_Шаблон экспресс оценки`, copied from `assets/Шаблон Экспресс оценки.xlsx` and placed after `99_Правила планирования`, to create `<ЗНИ/Jira> Экспресс-оценка.xlsx`. Fill `E2:K2` with the root ticket, `E3:K3` with the root description, and write root/child/grandchild rows from row 17 using description merges `A:D`, `B:D`, `C:D`; map AN to `G`, DE to the existing template field `Design` in `H`, and BE/FE/QA to `I:K` without changing the source template.
...
- `04_Квартальный план!R2` is a shape-backed `Экспорт плана` button bound to `RunQuarterPlanExportBacklog`. It exports non-empty `tblPlanActive` rows to standalone `.xlsx`, writing only visible user columns `F:T`; the default filename is `<Команда>_<Год>_Q<Квартал>_квартальный план_<yyyy-mm-dd_hh-nn>.xlsx`.
- Keep the R2 export button caption Mac-safe: use the visible cell text for the shape caption and the existing `ChrW$` helper text for VBA-generated Cyrillic pieces instead of raw Cyrillic string literals.
